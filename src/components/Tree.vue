<template>
    <div class="treepicker_tree">
        ...
        <div class="treepicker_tree_header">
            ...
            <p class="description">Select a group to filter the entries.<br/>Once selected, groups will expand to show
                children, which you may select to filter the entries further.</p>
        </div>
...
<script>
    ...
    */
    ...
    import {groupBy, prop, compose, entries, map} from 'lodash/fp'

    const groupFilteredPrograms = groupBy(prop('Category'))

    function addFilteredCountToPrograms(programs, filteredGroups) {
        compose(
            map(([cat, p]) => {
                p.filteredCount = filteredGroups[cat] ? filteredGroups[cat].length : 0
            }),
            entries
        )(programs)
    }
    ...
    export default {
        ...
        watch: {
            filteredPrograms(filteredPrograms) {
                const groups = groupFilteredPrograms(filteredPrograms)
                addFilteredCountToPrograms(this.categories.programs, groups)
            }
        },
        computed: {
            filterActive() {
                return this.filteredPrograms.length !== this.programs.length
            },
            sortedPrograms() {
            ...
            */
            buildCategories: function () {
                ...
                for (let p of this.programs) {
                    const {Category, IsIndirect} = p;
                    let bucket = IsIndirect === 'true' ? indirects : programs;
                    ...
                    bucket[Category].programs.push(p);
                }
                // for each program, find the filtered programs
                // for(let category of Object.keys(programs)) {
